 ...
 */
public class ActivityDoRepairV2 extends BaseActivity implements LocationSource,
	...
	private Button btn_pause_workorder, btn_continue_workorder,
			btn_disable_workorder, btn_init_workorder,btn_apply_spare_part;
	// btn_quick_close;
	...
	private CheckBox cb_small_error;
	...
	public void onClick(View v) {
		...
		case R.id.btn_pause_workorder:
			...
			Util.startActivity(ActivityDoRepairV2.this,
		...
		case R.id.btn_quick_close:
			b.putString("title", "小故障快速关闭");
			b.putInt(
					"type",
					Constant.EnumWorkorderPauseStatus.PAUSE_STATUS_WORKORDER_QUICKCLOSE);
			Util.startActivity(ActivityDoRepairV2.this,
					ActivityPauseReason.class, b, false);
			break;
		...
		case R.id.btn_close_assist:
			...
			intent = new Intent(ActivityDoRepairV2.this, CaptureActivity.class);
			intent.putExtras(b);
	...
	public void init(Bundle savedInstanceState) {
		...
		cb_small_error = (CheckBox) findViewById(R.id.cb_small_error);
		cb_small_error
	...
	private void BindWorkorderData() {
		...
		if (GlobalData.curWorkorder.Status == Constant.EnumWorkorderStatus.STATUS_WORKORDER_INVALID) {
			...
			if (GlobalData.curWorkorder.StartType.equals("1")) {
				tv_workorder_status.setText(tv_workorder_status.getText()
						+ ("(小故障)"));
				btn_complete_workorder_qr.setText("小故障快速关闭\n(扫描二维码)");
			} else {
				btn_complete_workorder_qr.setText("提交抢修反馈\n(扫描二维码)");
			}
		}
		...
		try {
			tv_workorder_desc.setText(Html.fromHtml("["
			...
			tv_workorder_desc.setMovementMethod(LinkMovementMethod
					.getInstance());
	...
	private void updateWorkorderStatus(String status) {
		String startType = cb_small_error.isChecked() ? "1" : "0";
		callBackUpdateStatus.setExtra(status);
		new SJECHttpHandler(callBackUpdateStatus, this).updateWorkorderStatusForRepair(
				workorder_id, status, latlng == null ? "0" : latlng.latitude
						+ "", latlng == null ? "0" : latlng.longitude + "",
				getText(et_workorder_remark), startType,resultType);
	}
	...
	private AsyncHttpCallBack callBackUpdateStatus = new AsyncHttpCallBack() {
		...
		public void onFailure(String message) {
			...
			if (getExtra().toString().equals(
				...
				unsubmit.Type = Constant.EnumWorkorderType.TYPE_WORKORDER_REPAIR;
	...
	private void BindPushLog() {
		...
		LayoutInflater mInflater = LayoutInflater.from(ActivityDoRepairV2.this);
	...
	private void disabledAllButton() {
		cb_small_error.setVisibility(View.GONE);
	...
	private void initButtonReceive() {
		if (isMeReceiver()) {
			// 如果单接单的人是自己
			cb_small_error.setVisibility(View.VISIBLE);
