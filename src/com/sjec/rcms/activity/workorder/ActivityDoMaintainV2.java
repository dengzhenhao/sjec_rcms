...
import com.sjec.rcms.data.Constant.EnumMaintainType;
...
public class ActivityDoMaintainV2 extends BaseActivity implements LocationSource, AMapLocationListener {
	...
	// 点受理时，跳出提示框，选择维保性质
	private LinearLayout layout_select_maintain_type;
	private TextView tv_season_maitain, tv_half_year_maitain;
	private CheckBox cb_half_month, cb_season, cb_half_year, cb_year;
	...
	public void onClick(View v) {
		...
		case R.id.btn_receive_workorder:
			layout_select_maintain_type.setVisibility(View.VISIBLE);
			break;
		case R.id.btn_confirm_receive:
		...
		case R.id.btn_workorder_pics:
			...
			break;
		case R.id.btn_apply_spare_part:
			Util.startActivity(this, ActivityApplySparePart.class, false);
			break;
		case R.id.cb_half_month:
			if (cb_half_month.isChecked()) {
				maintainType = v.getTag().toString();
			} else {
				maintainType = "";
			}
			cb_season.setChecked(false);
			cb_half_year.setChecked(false);
			cb_year.setChecked(false);
			break;
		case R.id.cb_season:
			if (cb_season.isChecked()) {
				maintainType = v.getTag().toString();
			} else {
				maintainType = "";
			}
			cb_half_month.setChecked(false);
			cb_half_year.setChecked(false);
			cb_year.setChecked(false);
			break;
		case R.id.cb_half_year:
			if (cb_half_year.isChecked()) {
				maintainType = v.getTag().toString();
			} else {
				maintainType = "";
			}
	...
	public void initLayout() {
		setContentView(R.layout.activity_workorder_do_maintain);
	}
	...
	public void init(Bundle savedInstanceState) {
		...
		btn_apply_spare_part = (Button) findViewById(R.id.btn_apply_spare_part);

		btn_continue_workorder = (Button) findViewById(R.id.btn_continue_workorder);
		btn_delay_workorder = (Button) findViewById(R.id.btn_delay_workorder);
		...
		// 选择维保性质
		layout_select_maintain_type = (LinearLayout) findViewById(R.id.layout_select_maintain_type);
		tv_season_maitain = (TextView) findViewById(R.id.tv_season_maitain);
		tv_half_year_maitain = (TextView) findViewById(R.id.tv_half_year_maitain);
		cb_half_month = (CheckBox) findViewById(R.id.cb_half_month);
		cb_season = (CheckBox) findViewById(R.id.cb_season);
		cb_half_year = (CheckBox) findViewById(R.id.cb_half_year);
		cb_year = (CheckBox) findViewById(R.id.cb_year);
		btn_confirm_receive = (Button) findViewById(R.id.btn_confirm_receive);
		btn_confirm_receive.setOnClickListener(this);
		cb_half_month.setOnClickListener(this);
		cb_season.setOnClickListener(this);
		cb_half_year.setOnClickListener(this);
		cb_year.setOnClickListener(this);
		...
		btn_delay_workorder.setOnClickListener(this);
		btn_quick_close.setOnClickListener(this);
		btn_apply_spare_part.setOnClickListener(this);
	...
	public void bindData() {
		...
		tv_title.setText(R.string.title_maintain);
	...
	private void BindWorkorderData() {
		...
		et_workorder_remark.setText(GlobalData.curMaintain.MaintainDesc);
		...
		tv_workorder_desc.setText(GlobalData.curWorkorder.Remark);
	...
	private AsyncHttpCallBack callBackUpdateRecive = new AsyncHttpCallBack() {
	...
	private void BindAssistWorker() {
		...
		if (GlobalData.curWorkorder.ChargeUserID != null && !GlobalData.curWorkorder.ChargeUserID.isEmpty()
			...
			if (isAllAssistClosed
				...
				changeButtonBg(btn_complete_workorder_qr, true);
			} else {
				changeButtonBg(btn_complete_workorder_qr, false);
	...
	private void disabledAllButton() {
		...
		changeButtonBg(btn_delay_workorder, false);
		changeButtonBg(btn_continue_workorder, false);
		changeButtonBg(btn_quick_close, false);
		changeButtonBg(btn_apply_spare_part, false);
		...
		if (isMeReceiver()) {
			...
			if (GlobalData.curWorkorder.OrderPause == Constant.EnumWorkorderPauseStatus.PAUSE_STATUS_WORKORDER_CONTINUE) {
				if (GlobalData.WorkerStatus == 1) {
					...
					// 正常模式
					// if (GlobalData.curWorkorder.StartType != null
					// && GlobalData.curWorkorder.StartType.equals("1")) {
					// // 小故障关闭
					// changeButtonBg(btn_complete_workorder_qr, true);
					// } else {
					// 非小故障,那么需要协助人员全部关闭后才可以关闭
					if (isAllAssistClosed) {
						changeButtonBg(btn_complete_workorder_qr, true);
					}
					// }
	...
	private void initControlStatusButton(boolean isBegin) {
		if (isMeReceiver()) {
			if (isBegin) {
				changeButtonBg(btn_apply_spare_part, true);
			}
			// 如果单接单的人是自己
			if (GlobalData.curWorkorder.OrderPause == Constant.EnumWorkorderPauseStatus.PAUSE_STATUS_WORKORDER_MAINTAIN_CHANGEDATE) {
				// 如果是暂停，
				if (isBegin) {
				...
				if (isBegin) {
					changeButtonBg(btn_delay_workorder, true);
					changeButtonBg(btn_quick_close, true);
				}
			}
